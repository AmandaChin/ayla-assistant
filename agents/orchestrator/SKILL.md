...
- `schemas/feishu-chat-daily-drop.schema.json`：远端飞书 Bot 按天写入聊天 JSON 的文件中转契约。
...
## Ayla Drop v1：飞书聊天日文件

远端 OpenClaw / 飞书 Bot 无法直接访问本机 Ayla API 时，使用文件中转协议。Bot 只产出标准 JSON 文件，Ayla 本地 importer 负责定时扫描、校验、去重，并转换为 `SourceEvent` / `InboxItem`。
...
- 按消息发生日创建 `incoming/YYYY-MM-DD/`，不要按同步日归档。
- 文件名使用 `{YYYYMMDD}T{HHmmss}-{producer_id}-{chat_id_or_mix}-{batch_id}.json`。
- 远端写入方先写 `tmp/`，完成后原子 rename 到 `incoming/YYYY-MM-DD/`，避免 Ayla 读取半截文件。
- 使用“按天目录 + 小批量 JSON 文件”，不要持续追加一个全天大 JSON。
- 每条消息必须带稳定 `message_id` 或 `idempotency_key`，用于去重。
- 契约文件见 `schemas/feishu-chat-daily-drop.schema.json`，样例见 `examples/feishu-chat-daily-drop.example.json`。
...
title       = chat.chat_name + " / " + sender.name
content     = plain_text || content
author      = sender.name
metadata    = chat / sender / mentions / labels / ayla_hints / raw
```

Bot 只负责准确写文件；不要自行修改 `processed/` 或 `failed/`。Ayla importer 处理成功后移动到 `processed/`，失败后移动到 `failed/` 并生成同名 `.error.json`。
